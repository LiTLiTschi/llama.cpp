...
static_assert(GGML_OP_COUNT == 99, "GGML_OP_COUNT != 96");
...
// ggml_moe_up_gate — fused MoE up+gate projection
...
struct ggml_tensor * ggml_moe_up_gate(
    ...
    // Fallback: if gate weights are separate, use individual MUL_MAT_ID + fused mul
    if (as_gate && (as_up->type != as_gate->type || !ggml_are_same_shape(as_up, as_gate))) {
        ...
        result_gate = ggml_unary(ctx, result_gate, op);
        return ggml_mul(ctx, result_gate, result_up);
    }
    ...
    result->src[2] = b;
    result->src[3] = ids;
    result->src[4] = NULL;  // up_bias
    result->src[5] = NULL;  // gate_bias
